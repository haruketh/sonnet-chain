...
Schema v2 preserves the mission/count/activity fields and adds a bounded `writing` view from WRITING onward. Its poem is the durable referee-accepted poem state; verified current-room speech and protocol facts supply up to 40 public-safe writing activities. Canonical-roster aliases replace DIDs. Proposals never enter the displayed poem before acceptance.
...
During a separate reviewed activation, deploy the Web's v1/v2-compatible reader first, then enable schema v2 and change only the independent exporter cadence from 300 to 60 seconds. Browser polling remains 60 seconds. This implementation does not alter or install that schedule.
